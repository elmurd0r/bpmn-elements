...
function SequenceFlow(flowDef, {environment}) {
  if (!(this instanceof SequenceFlow)) {
    return new SequenceFlow(flowDef, {environment});
  }
  ...
  const eventBroker = this[eventBrokerSymbol] = EventBroker(this, {prefix: 'flow', durable: true, autoDelete: false});
...
SequenceFlow.prototype.publishEvent = function publishEvent(action, content) {
  ...
  this[brokerSymbol].publish('event', `flow.${action}`, eventContent, {type: action});
};
...
SequenceFlow.prototype.getApi = function getApi(message) {
  return FlowApi(this[brokerSymbol], message || {content: this.createMessage()});
};
...
SequenceFlow.prototype.shake = function shake(message) {
  ...
  if (content.id === this.targetId) return broker.publish('event', 'flow.shake.loop', content, {persistent: false, type: 'shake'});

  for (const s of message.content.sequence) {
    if (s.id === this.id) return broker.publish('event', 'flow.shake.loop', content, {persistent: false, type: 'shake'});
  }
